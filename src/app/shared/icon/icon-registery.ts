...
export class CfIconRegistery {
  ...
  private _fetchIcon(iconConfig: SvgIconConfig): Observable<string> {
    ...
    if (!this._httpClient) {
      throw new Error(`httpClient is not provide`);
    }
    ...
    // // TODO: add an ngDevMode check
    if (!url) {
      throw new Error(`No url for:${safeUrl}`);
    }
  ...
  private _setSvgAttributes(
    ...
    /**
     * Reboot css of boostrap break style.
     * Remove it when boostrap is delete.
     */
    svg.style.verticalAlign = 'inherit';

    svg.setAttribute('fit', '');
    svg.setAttribute('vertical-align', 'inherit');

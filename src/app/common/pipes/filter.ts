...
export class FilterPipe implements PipeTransform {
  public transform(
    ...
    for (const title of items) {
      ...
      const match = searchWithSpellingErrors(searchTerm.toLowerCase(), item.toLowerCase(), 3);
...
function searchWithSpellingErrors(query: string, title: string, tolerance: number): string | null {
